//Reference from http://www.codeproject.com/Articles/18794/Improved-and-Recoded-RibbonMenuButton
...
namespace CommonUI.Buttons
{
    public class RibbonMenuButton : Button
    {
        ...
        /// <summary>
        /// 重载Image方法，在构造Image对象的同时，创建一个GrayImage，这样做的好处就是界面显示的时候
        /// 所有的界面一起显示，而不是之前那种显示很慢的情况
        /// 缺点就是非常耗时间，尽量添加一个SplashWindow
        /// </summary>
        ...
        public void DrawImage(Graphics gr)
        {
            if (this.Image != null)
            {
                ...
                Image img = Enabled ? this.Image : GetGrayImage();

                switch (_imagelocation)
                {
                    case e_imagelocation.Left:
                        gr.DrawImage(img, new Rectangle(offsetx, offsety, imagewidth, imageheight));
                        break;
                    case e_imagelocation.Right:
                        gr.DrawImage(img, new Rectangle(this.Width - imagewidth - offsetx, offsety, imagewidth, imageheight));
                        break;
                    ...
                    case e_imagelocation.Bottom:
                        gr.DrawImage(img, new Rectangle(offsetx, this.Height - imageheight - offsety, imagewidth, imageheight));
                        break;
                    default:
                        break;
                }
        ...
        Bitmap GetGrayImage()
        {
            if (_InactiveImage == null && Image != null)
            {
                _InactiveImage = ConvertToGrayscale(new Bitmap(this.Image));
            }
        ...
        public Bitmap ConvertToGrayscale(Bitmap source)
        {
            ...
            //make the black color to be transparent
