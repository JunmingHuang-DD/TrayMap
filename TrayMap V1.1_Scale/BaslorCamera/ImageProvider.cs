...
namespace Vision.Camera
{
    ...
    public class ImageProvider
    {
        ...
        /// <summary>
        /// a static function to call Pylon.Initialize() and Pylon.Terminate() function
        /// </summary>
        class PylonInit
        {
            private static PylonInit _Instance;
            public static PylonInit Instance
            {
                get
                {
                    if (_Instance == null)
                    {
                        _Instance = new PylonInit();
                    }
            ...
            public Dictionary<string, uint> CameraIDs = new Dictionary<string, uint>();
            ...
            PylonEnumerateDevices() before creating a device. */
                int numDevices = (int)Pylon.EnumerateDevices();

                for (int i = numDevices - 1; i >= 0; i--)
                {
                    PYLON_DEVICE_HANDLE camera = Pylon.CreateDeviceByIndex((uint)i);

                    Pylon.DeviceOpen(camera, Pylon.cPylonAccessModeStream);
                    string cameraID = Pylon.DeviceFeatureToString(camera, Pylon.cPylonDeviceInfoDeviceIDKey);

                    Pylon.DeviceClose(camera);

                    CameraIDs.Add(cameraID, (uint)i);
                }
            }
        ...
        public class Image
        {
            public Image(int newWidth, int newHeight, Byte[] newBuffer, IntPtr bufferPtr, bool color)
            {
                Width = newWidth;
                Height = newHeight;
                Buffer = newBuffer;
            ...
            public readonly IntPtr BufferPtr; /* data buffer Ptr, used for halcon*/
        }
        ...
        protected PYLON_WAITOBJECTS_HANDLE m_hWaitObjs;            /* Wait objects. */
        protected PYLON_WAITOBJECT_HANDLE m_hWait;         /* Handle used for waiting for a grab to be finished. */
        protected PYLON_WAITOBJECT_HANDLE m_hWaitGrabStop;
        ...
        private static PylonInit _InitInstance = PylonInit.Instance;
        private CameraTriggerSource _TriggerSource;
        private int _GrabCount = -1;
        #endregion

        #region properties
        public double Exposure
        {
            get { return Pylon.DeviceGetFloatFeature(m_hDevice, "ExposureTimeAbs"); }
            set 
            {
                if (!m_open)
                {
                ...
                Pylon.DeviceSetFloatFeature(m_hDevice, "ExposureTimeAbs", value);
            }
        }

        public double ColorImprove_Green
        {
            get 
            {
                Pylon.DeviceFeatureFromString(m_hDevice, "BalanceRatioSelector", "Green");
                return Pylon.DeviceGetFloatFeature(m_hDevice, "BalanceRatioAbs");
            }
            set
            {
                Pylon.DeviceFeatureFromString(m_hDevice, "BalanceRatioSelector", "Green");
                Pylon.DeviceSetFloatFeature(m_hDevice, "BalanceRatioAbs", value);
            }
        }

        public double ColorImprove_Red
        {
            get 
            {
                Pylon.DeviceFeatureFromString(m_hDevice, "BalanceRatioSelector", "Red");
                return Pylon.DeviceGetFloatFeature(m_hDevice, "BalanceRatioAbs"); 
            }
            set
            {
        ...
        public int Width 
        {
            get { return (int)Pylon.DeviceGetIntegerFeature(m_hDevice, "Width"); }
            set { Pylon.DeviceSetIntegerFeature(m_hDevice, "Width", value); }
        }

        public int Height
        {
            get { return (int)Pylon.DeviceGetIntegerFeature(m_hDevice, "Height"); }
            set { Pylon.DeviceSetIntegerFeature(m_hDevice, "Height", value); }
        }
        ...
        {
            if (!PylonInit.Instance.CameraIDs.ContainsKey(id))
            {
                m_lastError = "No camera with ID " + id + " found";
                throw new Exception("No camera with ID " + id + " found");
            }

            /* Get a handle for the device and proceed. */
            Open(Pylon.CreateDeviceByIndex(PylonInit.Instance.CameraIDs[id]));
        }
        ...
        public void OneShot(int grabCount = 1, CameraTriggerSource trigger = CameraTriggerSource.None)
        {
            ...
            if (m_open && !m_grabThread.IsAlive) /* Only start when open and not grabbing already. */
            {
               ...
               /* Set up the grabbing and start. */
                m_numberOfBuffersUsed = (uint)grabCount;
                m_grabOnce = true;
        ...
        public void ContinuousShot(CameraTriggerSource trigger = CameraTriggerSource.None)
        {
            if (_TriggerSource != trigger)
            {
                Stop();
            }

            if (_TriggerSource == trigger && m_grabThread.IsAlive)
            {
                return;
            }
        ...
        public void Stop()
        {
            ...
            {
                //signal the grab thread call
                if (m_hWaitGrabStop != null)
                {
                    Pylon.WaitObjectSignal(m_hWaitGrabStop);
                }

                m_grabThreadRun = false; /* Causes the grab thread to stop. */
                m_grabThread.Join(200); /* Wait for it to stop. */
            }
        }
        ...
        protected void Open(PYLON_DEVICE_HANDLE device)
        {
            try
            {
                ...
                bool isAvail = Pylon.DeviceFeatureIsAvailable(m_hDevice, "EnumEntry_PixelFormat_Mono8");
                ...
                if (Pylon.DeviceFeatureIsWritable(m_hDevice, "GevSCPSPacketSize"))
                {
                    ...
                    Pylon.DeviceSetIntegerFeature(m_hDevice, "GevSCPSPacketSize", JumboSize);
                }
        ...
        {
            //set up wait objects
            m_hWaitObjs = Pylon.WaitObjectsCreate();
            m_hWaitGrabStop = Pylon.WaitObjectCreate();
            Pylon.WaitObjectReset(m_hWaitGrabStop);
            Pylon.WaitObjectsAdd(m_hWaitObjs, m_hWaitGrabStop);
            Pylon.WaitObjectsAdd(m_hWaitObjs, m_hWait);
            ...
            bool bAcquisitionStartAvailable = Pylon.DeviceFeatureIsAvailable(m_hDevice, "EnumEntry_TriggerSelector_AcquisitionStart");
            bool bFrameStartAvailable = Pylon.DeviceFeatureIsAvailable(m_hDevice, "EnumEntry_TriggerSelector_FrameStart");

            /* Set the software FrameStartTrigger mode */
            if (_TriggerSource == CameraTriggerSource.Software)
            {
                //  m_TriggerSelector
            ...
            else if (_TriggerSource == CameraTriggerSource.Hardware)/* Set the external FrameStartTrigger mode */
            {
                //  m_TriggerSelector
                if (bAcquisitionStartAvailable && bFrameStartAvailable)
                {
                    ...
                    Pylon.DeviceFeatureFromString(m_hDevice, "TriggerSelector", "FrameStart");
                    Pylon.DeviceFeatureFromString(m_hDevice, "TriggerMode", "On");
                    Pylon.DeviceFeatureFromString(m_hDevice, "TriggerSource", "Line1");
                    //m_TriggerSelector = "FrameStart";
                    Pylon.DeviceFeatureFromString(m_hDevice, "TriggerActivation", "RisingEdge");
                }
                else if (bAcquisitionStartAvailable && !bFrameStartAvailable)
            ...
            {
                /* Disable frame start trigger if not in Trigger Mode. */
        ...
        protected void Grab()
        {
            ...
            try
            {
                ...
                while (m_grabThreadRun) /* Is set to false when stopping to end the grab thread. */
                {
                    ...
                    //if (!Pylon.WaitObjectWait(m_hWait, 1500))
                    //{
                    //    lock (m_lockObject)
                    //    {
                    ...
                    uint woIndex = 0;
                    /* Wait for the next buffer to be filled. Wait up to INFINITE  ms. */
                    Pylon.WaitObjectsWaitForAny(m_hWaitObjs, (uint)0xFFFFFFFF, out woIndex);

                    if (woIndex == 0)
                    {
                        //stop called and no image grabbed
                ...
                //Console.WriteLine("Grab thread quit!");
            }
            catch (Exception e)
            {
        ...
        public void SendSoftTrigger()
        {
            if (m_open && m_grabThread.IsAlive) /* Only start when open and grabbing. */
            {
                if (_TriggerSource == CameraTriggerSource.Software)
                {
                    /* Enable frame start trigger if in Software Trigger mode. */
                    if (Pylon.DeviceFeatureIsAvailable(m_hDevice, "EnumEntry_TriggerSelector_FrameStart"))
                    {
                        Pylon.DeviceFeatureFromString(m_hDevice, "TriggerSelector", "FrameStart");
        ...
        protected void EnqueueTakenImage(PylonGrabResult_t grabResult)
        {
            ...
            {
                newGrabResultInternal.ImageData = new Image(grabResult.SizeX, grabResult.SizeY, buffer.Array, buffer.Pointer, grabResult.PixelType == EPylonPixelType.PixelType_RGBA8packed);
            }
            else /* Conversion is required. */
            {
                ...
                /* Add the image data. */
                newGrabResultInternal.ImageData = new Image(grabResult.SizeX, grabResult.SizeY, convertedBuffer.Array, convertedBuffer.Pointer, m_converterOutputFormatIsColor);
            }
            lock (m_lockObject) /* Lock the grab result queue to avoid that two threads modify the same data. */
            {
        ...
        protected void CleanUpGrab()
        {
            ...
            Pylon.WaitObjectsRemoveAll(m_hWaitObjs);
            Pylon.WaitObjectDestroy(m_hWaitGrabStop);
            Pylon.WaitObjectsDestroy(m_hWaitObjs);
        }
        ...
        #region camera notification events
        ...
        protected void OnImageReadyEvent()
        {
            ...
            {
                //ImageReadyEvent();
                ImageReadyEvent.BeginInvoke(null, null);
            }
        }
